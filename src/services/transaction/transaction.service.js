...
class TransactionService {
    ...
    try {
      ...
      return transactions.map((tx) => {
        ...
        return {
          ...
          status:
            tx.ret && tx.ret[0] && tx.ret[0].contractRet === "SUCCESS"
              ? "confirmed"
              : tx.block_number
              ? "failed"
              : "pending",
  ...
  async estimateFee(params) {
    ...
    if (chain.type === "EVM") {
      ...
      const estimatedFee = ethers.utils.formatEther(gasPrice.mul(gasLimit));
  ...
  async sendNative(params) {
    ...
    try {
      ...
      const network = wallet.networks.find(
        (n) => n.network.toUpperCase() === chainId.toUpperCase()
      );
      if (!network) {
  ...
  async sendToken(params) {
    ...
    try {
      ...
      if (chain.type !== "EVM") {
        throw new Error(
          "Token transfers only supported on EVM and TRON chains"
        );
      }
      ...
      const signer = new ethers.Wallet(
        credentials.privateKeys[chainId.toLowerCase()],
        provider
      );
      ...
      const estimatedGas =
        gasLimit ||
        (await tokenContract.estimateGas.transfer(toAddress, amountInUnits));
  ...
  async monitorTransaction(txId, txHash, chainId) {
    ...
    if (chain.type === "EVM") {
      try {
        ...
        console.error(
          `Transaction monitoring failed for ${txHash}:`,
          error.message
        );
        await transactionRepository.update(txId, {
          status: "failed",
  ...
  async monitorTronTransaction(txId, txHash) {
    try {
        ...
        try {
          ...
          if (txInfo && txInfo.blockNumber) {
            ...
            if (txInfo.blockNumber) {
              try {
                const block = await tronWeb.trx.getBlockByNumber(
                  txInfo.blockNumber
                );
            ...
            // Determine status
            const status =
              txInfo.receipt && txInfo.receipt.result === "SUCCESS"
                ? "confirmed"
                : "failed";
      ...
      console.error(
        `Tron transaction monitoring failed for ${txHash}:`,
        error.message
      );
      await transactionRepository.update(txId, {
        status: "failed",
  ...
  async estimateTronFee(params) {
    ...
    try {
      ...
      const txSize = transaction.raw_data_hex
      ...
      const bandwidthFee =
        availableBandwidth >= bandwidthNeeded
          ? 0
